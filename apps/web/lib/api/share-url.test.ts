...
function request(headers: Record<string, string> = {}): Request {
  return new Request("http://internal-web:3000/api/shared-files", { method: "POST", headers });
}

describe("shareUrlFor", () => {
  it("prefers the configured app url", () => {
    expect(shareUrlFor(request(), ID, "https://snapjaw.dev")).toBe(`https://snapjaw.dev/s/${ID}`);
  });

  it("strips trailing slashes from the configured url", () => {
    expect(shareUrlFor(request(), ID, "https://snapjaw.dev///")).toBe(
      `https://snapjaw.dev/s/${ID}`,
    );
  });
  ...
  it("uses the forwarded host and proto when no app url is set", () => {
    ...
    expect(url).toBe(`https://snapjaw.dev/s/${ID}`);
  });
  ...
  it("falls back to the request origin in development", () => {
    expect(shareUrlFor(request(), ID, undefined)).toBe(`http://internal-web:3000/s/${ID}`);
  });
  ...
  it("lets the configured url win over forwarded headers", () => {
    ...
    expect(url).toBe(`https://a.dev/s/${ID}`);
  });
});
